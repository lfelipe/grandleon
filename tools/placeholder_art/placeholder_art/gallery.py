...
its sprite, its motion, its recolourings and its mesh side by side. Judging one
...
from . import (autotile, backdrops, characters, figures, frames, gltf, meshes,
               pixelfont, playstation_header, pngio, preview3d, profiles,
...
#: the mesh alike. It is the colour the export bakes, which is the colour every
#: other measurement in this pipeline is taken on, and naming it once here is
#: what keeps a row's two halves from being drawn in two different factions.
UNIT_FACTION = gltf.BAKED_FACTION
...
def _mesh_panel(archetype: str, preview: preview3d.Preview) -> Sheet:
    """One exported mesh, drawn at the shipped camera and labelled a preview.

    The figure is upscaled by nearest neighbour exactly as every sprite on
    these pages is, and for the same reason: what the console computes is a
    figure about thirty pixels tall, and smoothing it up would be showing
    something the machine never draws.
    """
    zoom = preview3d.PREVIEW_ZOOM
    width = preview.width * zoom + 16
    height = preview.height * zoom + 24
    sheet = Sheet(width, height)
    sheet.rect(6, 6, preview.width * zoom + 4, preview.height * zoom + 4, PANEL)
    for y in range(preview.height):
        for x in range(preview.width):
            colour = preview.at(x, y)
            if colour is None:
                continue
            sheet.rect(8 + x * zoom, 8 + y * zoom, zoom, zoom,
                       (colour[0], colour[1], colour[2], 255))
    sheet.text(f"{archetype} mesh preview".upper(), 8,
               height - 13, DIM, 1)
    return sheet
...
def _roster_markdown(units: Dict[str, str], models: Dict[str, str]) -> str:
    """``ROSTER.md``: one row per unit, its 2D beside its 3D.

    The counts are measured off ``models`` rather than stated, so a mesh
    commissioned later fills its cell and moves the numbers with no edit here.
    """
    ...
    lines: List[str] = [
        ...
        "<!-- Generated by tools/placeholder_art/generate.py. Do not edit. -->",
        "",
        "One row per unit: every archetype in every style, its two-dimensional "
        "art beside its three-dimensional art. `GALLERY.md` cuts the same",
        ...
        f"- **3D: {drawn} of {total}.** A mesh is an *additional* drawing of an "
        "archetype and not a cell of the sequence every style must ship, so an "
        "archetype",
        "  without one is not an incomplete archetype. But it is an empty "
        "cell here, and the empty cells are half of what this page is for.",
        "",
        "| Style | 2D units | Meshes |",
        "| --- | --- | --- |",
    ]
    for style in styles.STYLES:
        count = sum(1 for archetype in characters.ARCHETYPE_ORDER
                    if f"{style.name}_{archetype}" in models)
        lines.append(
            f"| {style.label} | {len(characters.ARCHETYPE_ORDER)} of "
            f"{len(characters.ARCHETYPE_ORDER)} | {count} of "
    ...
    lines += [
        ...
        "pushed the 3D half of the row off the side of the page.",
        "",
        ...
        "",
        "Each mesh is rendered **from the exported model** "
        "(`assets/gltf/<style>/<archetype>.gltf` and its buffer) and not from "
        "the part tables the",
        "exporter wrote it from. The corners, the per-face normals, the "
        "far-to-near part order and each face's ramp and rung all come out of "
        "the file; the",
        "ramp's colours come from the same resolution the console's own header "
        "is built with. So the picture is an end-to-end check on the export as "
        "well as",
        "on the art: a model that lost a part, smoothed a normal or shuffled "
        "its order comes out as a wrong picture, which is the one failure a "
        "round trip",
        "over the same integers cannot notice.",
        "",
        "The camera is the shipped one: "
        f"{meshes.PITCH_DEGREES} degrees off the board plane, the shipped "
        "focal length and distance. A mesh that reads at another angle is not",
        "evidence about the one the game draws. At that camera the machine "
        "draws a figure between twenty-five and forty-one pixels tall "
        "depending where on the board it stands, so these are upscaled by "
        "nearest",
        f"neighbour {preview3d.PREVIEW_ZOOM} times, exactly as every sprite on "
        "these pages is. The outline under each figure is one board tile at "
        "the same camera.",
        "",
        "**These are previews and not screenshots.** The console draws these "
        "meshes for real in "
        "`platform/playstation/scratch/grandleon_playstation_scratch3d`,",
        "and `platform/playstation/scratch/evidence/` holds its own "
        "photographs of them on a board. This renderer was checked against "
        "those captures rather than",
        "trusted: redrawing `evidence/mesh-beside-billboard.png` with the same "
        "projection puts the knight 25, 37 and 41 pixels tall at the near, "
        "middle and far",
        "rows, which are the three heights that still's own notes record, and "
        "88 per cent of the pixels it paints carry one of the model's own face "
        "colours there. Two differences remain and",
        "both are deliberate: the console additionally scales each unit to its "
        "own sprite's opaque silhouette at draw time, which is a property of "
        "placing a unit",
        "on a board rather than of the model, and its rasteriser is its GPU's "
        "rather than this one's. An edge pixel here and there falls on the "
        "other side.",
        "",
        "## The roster",
        "",
    ...
    for style in styles.STYLES:
        for archetype in characters.ARCHETYPE_ORDER:
            ...
            cell = (f"![{key} mesh preview]({model})" if model
                    else "*no mesh commissioned*")
            lines.append(
                f"| {style.label} | **{label}** | "
                f"![{key}]({units[key]}) | {cell} |")
    return "\n".join(lines) + "\n"
...
def _write_roster(output: Path,
    ...
    Which meshes exist is asked of the **exported files**, not of the mesh
    registry: three commissions can land in the time it takes to write a page,
    and a page that read the registry would have to be edited when they did.
    """
    directory = output / "gallery"
    reference = converted[profiles.PROFILES[0].name]
    # The profile the mesh ramps are resolved from is the one the PlayStation
    # header is emitted from, because the ramps are that header's CLUTs and a
    # second opinion about them is exactly what this page must not become.
    source = converted[playstation_header.SOURCE_PROFILE]
    units: Dict[str, str] = {}
    models: Dict[str, str] = {}
    written = 0
    for style in styles.STYLES:
        exported = output / gltf.relative_directory(style.name)
        for archetype in characters.ARCHETYPE_ORDER:
            key = f"{style.name}_{archetype}"
            ...
            written += 1
            if not preview3d.available(exported, archetype):
                continue
            ramps = preview3d.ramp_colours(
                playstation_header.mesh_ramp_words(source, style, archetype,
                                                   UNIT_FACTION),
                playstation_header.clut_channels)
            preview = preview3d.render(preview3d.load(exported, archetype),
                                       ramps)
            filename = f"mesh_{key}.png"
            pngio.write(directory / filename,
                        _mesh_panel(archetype, preview).encode())
            models[key] = f"gallery/{filename}"
            written += 1
    (output / "ROSTER.md").write_text(_roster_markdown(units, models),
                                      encoding="utf-8")
    return written + 1
