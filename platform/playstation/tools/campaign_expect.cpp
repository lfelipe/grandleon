...
client::CampaignSessionError play(
    ...
    const char* slot_base, storage::SlotStorage& device, turn::TurnClient& host
) {
    ...
    // The name the slot screen carries, read off the package rather than taken
    // as an argument, because that is where the executable reads it from. A
    // title this tool was *told* would derive the screen the console is then
    // required to draw, which is the one input a derivation must not have.
    const turn::TurnClient::SlotChoice chosen = host.open_campaign(
        pr::project_title(package), slot_base, holds, view::slot_menu_rows
    );
...
int main(int argc, char** argv) {
    if (argc != 6) {
        std::cerr
            ...
            << " <project.json> <campaign-path> <slot-base>"
               " <found|resume> <expectations.txt>\n";
        return 64;
    }
    const std::string campaign_path = argv[2];
    const std::string slot_base = argv[3];
    ...
    if (mode == "record") {
        ...
        const client::CampaignSessionError played = play(
            loaded.package, campaign_id, slot_base.c_str(), device, recorder
        );
    ...
    if (mode == "resume") {
        ...
        const client::CampaignSessionError first = play(
            loaded.package, campaign_id, slot_base.c_str(), device, warm
        );
    ...
    const client::CampaignSessionError status = play(
        loaded.package, campaign_id, slot_base.c_str(), device, host
    );
